...
export default class SpecialSelections extends Component {
    ...
    render() {
        return (
            ...
            <select className='select-sel' placeholder={this.state.gender} onChange={this.filterSpecialSelection}>
         ...
         <button onClick={() => this.scroll(-345)} className='prev'><Prev/></button>
         {this.state.items.map((item)=>(
             <ProductCart key={item.id} product={item}/>
         ))}
         <button onClick={() => this.scroll(345)} className='next'><Prev/></button>
         </div>
         </div>
