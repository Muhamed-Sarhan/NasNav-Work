...
const OnSale = ({ gender , filterOnSaleSection , filteredDataOnSale , onSaleRef}) => {
    ...
    return (
        <div className='bestSellers'>
         ...
         <div className='bestsellerHeader'>
            ...
            <h3 className='caption'>Hurry up! catch these products before the sale ends.</h3>
            </div>
            <div className='bestButtons'>
            <div className="custom-select">
            <select className='select-sel' placeholder={gender} onChange={filterOnSaleSection}>
        ...
        <button onClick={() => scroll(-345)} className='prev'><Prev/></button>
        {filteredDataOnSale.map((product)=>(
            <ProductCart key={product.id} product={product}/>
        ))}
        <button onClick={() => scroll(345)} className='next'><Prev/></button>
        </div>
        </div>
