    ...
    return (
        <div className='bestSellers'>
         <div className='bestsellerHeader'>
            ...
            <select className='select-sel' placeholder={gender} onChange={filterByGender}>
         ...
         <button onClick={() => scroll(-340)} className='prev'><Prev/></button>
         {filteredData.map((product)=>(
             <ProductCart key={product.id} product={product}/>
         ))}
         <button onClick={() => scroll(340)} className='next'><Prev/></button>
         </div>
         </div>
